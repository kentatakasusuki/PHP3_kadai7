...
?>  
		...
		<div class="col-md-10">
<h2>モデル編集</h2>
<? foreach($arg["data2"] as $data){ //ここからデータを吐き出し
	
$blod_sel[$data["bloodtype"]] ="selected";//selectedをつけることでデータベースでセレクタのデータを出せる
$banktype_sel[$data["banktype"]] ="selected";//selectedをつけることでデータベースでセレクタのデータを出せる
...
<form action="editaction.php" method="POST" class="form-horizontal">		
	...
	<div class="form-group">
		...
		<div class="col-sm-1">
		<select class="form-control" id="InputSelect" name="cont5"><!--セレクタがついてるところはselected-->
...
<textarea name="cont14" id="" cols="30" rows="10" class="form-control"><?=$data["intro"] ?></textarea>
	...
	<input type="hidden" name="id" value="<?=$data["id"] ?>"><!--IDを持っていくために裏で見えているデータ・hidden処理-->
